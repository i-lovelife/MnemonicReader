...
def vectorize(ex, model, single_answer=False):
    ...
    def get_full_char(word):
        ret = [char_dict[c] for c in word]
        length = len(ret)
        if length > args.max_char_len:
            ret = ret[:args.max_char_len]
        ret += [char_dict[char_dict.NULL]] * (args.max_char_len - length)
        return ret
    # Index words
    document = torch.LongTensor([word_dict[w] for w in ex['document']])
    question = torch.LongTensor([word_dict[w] for w in ex['question']])
    if args.full_char:
        document_char = torch.LongTensor([get_full_char(w) for w in ex['document']])
        question_char = torch.LongTensor([get_full_char(w) for w in ex['question']])
    else:
...
def make_batchify_func(args):
    def batchify(batch):
        ...
        if args.full_char:
            x1_c = torch.LongTensor(len(docs), max_length, args.max_char_len).zero_()
        ...
        if args.full_char:
            x2_c = torch.LongTensor(len(questions), max_length, args.max_char_len).zero_()
